...
export default function HomeScreen() {
  ...
  const [nacionalidadCustom, setNacionalidadCustom] = useState('');
  const [propositoCustom, setPropositoCustom] = useState('');
...
const handleDateChange = (event, selectedDate) => {
  ...
  const handleSelectOption = (option) => {
    ...
    if (dropdownData.field === 'Nacionalidad' && option !== 'Otra') {
      setNacionalidadCustom('');
    }
    if (dropdownData.field === 'PropositoPrestamo' && option !== 'Otro') {
      setPropositoCustom('');
    }
  ...
  const handleGuardar = async () => {
    ...
    if (!validarIdentificacion(persona.TipoIdentificacion, persona.NumeroIdentificacion)) {
      toast.show({
        type: 'error',
        text1: 'Identificación no válida',
        text2: 'Número de identificación no válido para el tipo seleccionado.',
      });
      return;
    }
    ...
    try {
      ...
      console.error('Error al guardar informaci�n:', err?.response?.data || err);
      let mensaje = 'Verifica los datos o la conexi�n.';
        ...
        <Card style={styles.card}>
          ...
          {activeSection === 0 && (
            <View style={styles.sectionContent}>
              ...
              {renderDropdown('nacionalidad', 'Nacionalidad', dropdownOptions.Nacionalidad, 'persona', 'Nacionalidad')}
              {persona.Nacionalidad === 'Otra' && (
                <TextInput
                  label="Especifica nacionalidad"
                  mode="outlined"
                  value={nacionalidadCustom}
                  onChangeText={(text) => {
                    setNacionalidadCustom(text);
                    handleInput('persona', 'Nacionalidad', text);
                  }}
                  style={styles.input}
        ...
        <Card style={styles.card}>
          ...
          {activeSection === 2 && (
            <View style={styles.sectionContent}>
              ...
              {solicitud.PropositoPrestamo === 'Otro' && (
                <TextInput
                  label="Especifica el prop�sito"
                  mode="outlined"
                  value={propositoCustom}
                  onChangeText={(text) => {
                    setPropositoCustom(text);
                    handleInput('solicitud', 'PropositoPrestamo', text);
                  }}
                  style={styles.input}
